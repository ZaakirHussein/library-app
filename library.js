...
const closeForm = () => {
  ...
  const resetInvalidInputMessage = (() => {
    const inputs = Array.from(formInput.elements);
    inputs.forEach((i) => {
...
const displayBooks = (newBook) => {
  ...
  const rowContainer = tableContainer.appendChild(
  ...
  if (newBook.status == false) {
    const statusToggle = statusColumn.appendChild(
  ...
  const trashContainer = rowContainer.appendChild(
    document.createElement("div")
  );
...
const updateLibrary = () => {
  ...
  libraryItems.forEach((libraryItems) =>
    libraryHeader.removeChild(libraryItems)
  );
